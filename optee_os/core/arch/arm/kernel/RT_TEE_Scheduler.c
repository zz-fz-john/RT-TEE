...
static void rt_tee_thread_invoke(uint32_t arg_session_id, int taskUid)
{
	...
	msg_arg->func=0;//msg_arg->func：指定命令的具体功能编号，这里是 0。调用TA中编号为0的功能
	...
	for(int i = 0; i < SECURE_WORLD_MAX_SUPPORTED_TASKS; i++){//调度结束，会返回normal world，且调度器停止，估计bug就出现在这个地方。
		...
		if(secure_tasks[i].occupied && secure_tasks[i].cpu == coreNum){
			if(secure_tasks[i].numOfJobCompleted >= NUM_OF_MEASURED \
				|| getCurrentTime_micro() - start_time[get_current_core()] > NUM_OF_MEASURED * secure_tasks[i].period)//如果任务已经完成了NUM_OF_MEASURED个周期或者超出了NUM_OF_MEASURED* secure_tasks[i].period的时间，表示超时了
			{
				...
				if(getCurrentTime_micro() - start_time[get_current_core()] > NUM_OF_MEASURED * secure_tasks[i].period \
					...
					&& !miss_all_flag[i])
					{
					secure_tasks[i].numOfJobDlMissed += NUM_OF_MEASURED - secure_tasks[i].numOfJobCompleted;//表示该任务超出了deadline
					miss_all_flag[i] = 1;
					}
				continue;
			}
